  ...
  const status_type_widgets_modal_print = (html, data) => {
    ...
    status_logs.forEach(
      device_status => {
        // tbody html
        tbody_html += `<tr>`;
        tbody_html += `<td class="align-middle"><div class="d-flex align-items-center gap-2"><button data-bs-target="#card-widget-note-modal-${device_status.id}" data-bs-toggle="modal" class="btn btn-sm btn-success"><i class="ph-eye"></i></button></div></td>`;
        ...
        // modal class
        let subscribetopic = device_status.device.subscribe_topic;
        let subscribetopicarray = subscribetopic.split('/');
        let subscribetopicclass = subscribetopicarray[0] +
          '-' + subscribetopicarray[1] +
          '-' + subscribetopicarray[2];
        cloned_card_widget_note_modal_html.addClass(
          'card-widget-note-modal-' + subscribetopicclass
        );
        ...
        // streaming button
        cloned_card_widget_note_modal_html
          .find('.modal-footer')
          .prepend(
            `<button class="btn btn-sm btn-success card-widget-note-modal-stream-button">
        ...
        // close listener to note modal
        cloned_card_widget_note_modal_html.on('hide.bs.modal', async function(e) {
          const data = {
            '_token': '{{ csrf_token() }}',
            'device_id': device_status.device_id
          };
          await $.ajax({
            url: '/admin/devices/publish-streaming-stop',
            type: 'POST',
            data: data,
        ...
        // event listener for stream button
        var stream_action = document.querySelector(
          '#card-widget-note-modal-' +
          device_status.id +
          ' .card-widget-note-modal-stream-button'
        );
        if (stream_action) {
          stream_action.addEventListener('click', () => {
            $('#card-widget-note-modal-' + device_status.id + ' .modal-content .modal-streaming').html(`
              <div 
                style="aspect-ratio: 16/9; display: flex" 
