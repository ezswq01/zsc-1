        ...
        <div class="col-lg-8 col-12">
          <form action="{{ route('admin.devices.store') }}" method="POST">
            ...
            <div class="row mb-3">
              ...
              <div class="col-lg-8">
                <input class="form-control" value="{{ $data->id }}" name="id" placeholder="Type Device ID"
                  required type="text">
              </div>
            </div>
            <div class="row mb-3">
...
@push('js')
  <script>
    const dataSubscribeExpressions = @json($data->subscribe_expressions)
    const dataPublishActions = @json($data->publish_actions)
    ...
    $(document).ready(function() {
      if (dataSubscribeExpressions.length > 0) {
        dataSubscribeExpressions.forEach((item, index) => {
          ...
          $(`.subscribe_expression_${index + 1} select[name="subscribe_expressions[status_type][]"]`).val(item.status_type);
        })
      }
